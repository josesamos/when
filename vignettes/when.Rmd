...
The rest of this document is structured as follows: First, the general process of defining these dimensions is presented. Next, a section is dedicated to showing examples of dimension definitions, time-based and date-based dimensions. Finally, the document ends with conclusions and bibliography.
...
```{r}
w_date_7 <- w_date |>
  configure_month_level(exclude_all = TRUE, month_name = TRUE)
...
In the case of the date-based dimension levels, we can select or deselect them directly using the `configure_date_levels()` function. It allows us to treat the levels globally in the same way that we have been treating the attributes. For example, to exclude the month level we can do the following.
