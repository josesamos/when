...
For example, if we do not want to include the name of the month, we can configure it using the `configure_month_level()` function or at the time of object creation with a parameter of the same name, as shown below.
...
There is available a similar function for each level: `configure_time_level()`, `configure_day_level()`, `configure_week_level()`, `configure_month_level()` and `configure_year_level()`.

For date-based levels, we can include or exclude them using the `level` parameter of each of those functions or using the `configure_date_levels()` function. The advantage of this last function is that we can include or exclude several levels in a single step.
...
w_date_8 <- w_date |>
  configure_date_levels(month_level = FALSE)
...
If we want to include the level with the default attributes, just use the `configure_month_level()` function again with the default values of the parameters.
