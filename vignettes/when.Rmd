...
The rest of this document is structured as follows: The general process of defining these dimensions is presented. Then, the document ends with conclusions and bibliography.
...
We can configure the attributes it includes using the `configure_time_level()` function. The attribute called "hour" must always be included. If we do not include the "minute" attribute then "second" will not be included either. Below are some examples of definition and the result obtained.
...
`when` package offers a set of operations that allow us to
