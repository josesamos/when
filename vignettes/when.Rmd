...
For days, in addition to the language of the name of the day of the week, we can define the start day of the week. There are two possibilities: that the first day of the week is Monday or that it is Sunday. This influences the numbering of the days.
...
The week number associated with each date depends on the type of date dimension selected: standard (NULL), ISO 8601 ('iso') or epidemiological ('epi').
...
The epidemiological week is like ISO 8601 only that it considers that the week begins on Sunday, it numbers blocks from Sunday to Monday.


### Inclusion of the date associated with the week
...
We can get the definition function of any attribute using the `get_attribute_definition_function()` function. Below is the function for the "year" attribute.
...
In view of the function, we can define one with the same structure and establish it as a new attribute definition function using the `set_attribute_definition_function()` function. For example, below shows how to change the year attribute definition function using another of the functions from the [`lubridate`](https://CRAN.R-project.org/package=lubridate) package.

```{r}
f <- function(table, values, ...) {
  table[['year']] <- as.character(lubridate::epiyear(values))
  table
}
