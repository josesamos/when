...
If it is generated in the form of a string (`as_string = TRUE`, default option), the result can be used to easily change the name of the attributes, when the table is generated, using the `set_table_attribute_names()` function.

We have a function to configure each level: the attributes defined by default are configured there. We can select or deselect each attribute individually using the parameter named "include_" and the attribute name. We can also select all attributes defined at the level using the special `include_all` parameter. We can also include or uninclude a level using the `include_level` parameter of each function.
